...
def pipeline_to_signals(features_row, preprocessor=None) -> dict:
    """
    Convert a per-nurse feature row from engineer_features() into the 7
    behavioral QuietSignals signal values.

    This lets the richer TILES-2018 feature set feed into the unified
    composite scorer in predict.py.

    Parameters
    ----------
    features_row : dict or pd.Series — one row from the engineer_features() output
    preprocessor : optional sklearn Pipeline (imputer + scaler) — pass to
                   inverse-transform if your row is already scaled

    Returns
    -------
    dict with keys matching the 7 behavioral SIGNALS in signals.py
    """
    def get(key, default):
        v = features_row.get(key, default) if isinstance(features_row, dict) else getattr(features_row, key, default)
    ...
    # gsr ← HRV (low HRV = high stress)
    hrv = get("hrv_rmssd_mean", 35)
    gsr = clip(1.0 - (hrv - 10) / 45)

    # gait_irregularity ← step regularity (direct inverse)
    step_reg = get("step_regularity_mean", 0.5)
    gait_irregularity = clip(1.0 - (step_reg - 0.1) / 0.7)

    # voice_monotony ← sleep efficiency proxy
    sleep_eff = get("sleep_efficiency_mean", 80)
    voice_monotony = clip(1.0 - (sleep_eff - 40) / 58)

    # patient_rel ← sleep duration (inverse signal: more sleep → better relationships)
    sleep_hrs = get("sleep_duration_hrs_mean", 6)
    patient_rel = clip((sleep_hrs - 2) / 5.2)

    # task_switch ← sedentary bouts (fragmented movement ≈ fragmented attention)
    sed_bouts = get("sedentary_bouts_mean", None)
    if sed_bouts is not None:
        task_switch = clip((sed_bouts - 3) / 8)
    else:
        rhr = get("resting_hr_mean", 70)
        task_switch = clip((rhr - 58) / 18)

    # color_chaos ← sleep onset variability (irregular schedule → cognitive chaos)
    sov = get("sleep_onset_variability_mean", None)
    if sov is not None:
        color_chaos = clip((sov - 0.1) / 1.9)
    else:
        awake = get("awake_min_mean", 20)
        color_chaos = clip((awake - 15) / 25)

    # tiktok_burnout ← night awakenings (proxy for late-night phone use)
    n_awk = get("n_awakenings_mean", None)
    ...
    return {
        ...
        "voice_monotony":    voice_monotony,
        "gait_irregularity": gait_irregularity,
        "patient_rel":       patient_rel,
        "color_chaos":       color_chaos,
        "tiktok_burnout":    tiktok_burnout,
    }
